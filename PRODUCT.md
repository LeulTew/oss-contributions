...
Ordinary design choices are autonomous. First visits use light mode, including on a dark operating system. Explicit stored light, dark or system choices are preserved; dark mode uses neutral charcoal, not navy. Operational text is at least 14px, comment bodies 16px, and interactive targets at least 44px. Selected-PR previous/next navigation and reliable return-to-list focus reduce repeated scanning. The slash shortcut works from collection rows and other noneditable controls without interrupting typing or comboboxes.

The component refinement keeps that neutral identity but removes the explanatory detour: a compact command bar, segmented views and dense rows lead to one PR title, independent review/CI signals and conversation. Supporting summaries, full CI context and initial head baselines live in Details; they are not duplicate headings or fabricated conversation. A concise Gin inspection caveat remains visible even with Details closed. Empty discussion distinguishes unavailable evidence from hidden automation and offers an explicit way to reveal the latter.

A single Data & help disclosure explains refresh limits, privacy and historical semantics. Exact discussion and CI observation times remain visible in the selected view. The global Chi quotation appears only in the collection's Reviewer feedback disclosure, never as a testimonial on another selected PR. Public message bodies and curated quotations are unchanged. The local component revision must be inspected before any new commit or publication.
